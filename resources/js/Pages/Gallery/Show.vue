  ...
  <MainLayout>
    <div class="max-w-7xl mx-auto px-4 py-10">
      <div class="flex items-center gap-3 mb-2">
        <Link :href="route('gallery.index')" class="text-gray-400 hover:text-[#0d2f6e] transition">
          <ArrowLeftIcon class="w-5 h-5" />
        </Link>
        <h1 class="text-3xl font-bold text-[#0d2f6e]">{{ album.nom }}</h1>
      </div>
      <p v-if="album.description" class="text-gray-500 mb-6 ml-8">{{ album.description }}</p>
      <p v-else class="mb-6" />

      <!-- Filtres par type -->
      <div class="flex flex-wrap gap-3 mb-8">
        <Link :href="route('gallery.show', album.id)" class="px-4 py-2 rounded-full text-sm border transition" :class="!filtre_type ? 'bg-[#0d2f6e] text-white' : 'bg-white text-gray-600 hover:bg-gray-100'">Tout</Link>
        <Link :href="route('gallery.show', { album: album.id, type: 'photo' })" class="px-4 py-2 rounded-full text-sm border transition" :class="filtre_type === 'photo' ? 'bg-[#0d2f6e] text-white' : 'bg-white text-gray-600 hover:bg-gray-100'">Photos</Link>
        <Link :href="route('gallery.show', { album: album.id, type: 'video' })" class="px-4 py-2 rounded-full text-sm border transition" :class="filtre_type === 'video' ? 'bg-[#0d2f6e] text-white' : 'bg-white text-gray-600 hover:bg-gray-100'">Vidéos</Link>
      </div>

      <!-- Grille -->
      <div v-if="items.data.length" class="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
        <div v-for="item in items.data" :key="item.id" class="break-inside-avoid">
          <div
      ...
      <div v-else class="text-center text-gray-400 py-16">Aucun élément dans cet album.</div>
...
const page = usePage()
const filtre_type = computed(() => new URLSearchParams(window.location.search).get('type'))
